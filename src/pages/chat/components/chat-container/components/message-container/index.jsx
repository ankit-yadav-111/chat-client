...
useEffect(()=>{
  ...
  const checkImage=(filePath)=>{
    const imageRegex=
    /\.(jpg|jpeg|png|gif|bmp|webp|svg|tiff|tif|ico|heic|heif)$/i;
  ...
  const renderMessages = () => {
    ...
    return selectedChatMessages.map((message,index) => {
      const messageDate = moment(message.timesStamp).format("YYYY-MM-DD");
        ...
        <div key={index}>
          {showDate && (
            <div className="text-center text-gray-500 my-2">
              {moment(message.timesStamp).format("LL")}
            </div>
          )}
  ...
  const renderDMMessages = (message) => (
    ...
    >
      ...
      <div className="text-xs text-gray-600">
        {moment(message.timesStamp).format("LT")}
       ...
       >
        ...
        {message.sender._id !== userInfo.id ? (
          ...
          <Avatar className="h-8 w-8  rounded-full overflow-hidden">
                  ...
                  <span className="text-xs text-white/60">{moment(message.timesStamp).format("LT")}
                  </span>
                   </div> 
                   ):(
                  <div className="text-xs text-white/60 mt-1">{moment(message.timesStamp).format("LT")}</div>
                   )}
                   </div>
)
